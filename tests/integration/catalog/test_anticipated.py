...
async def seeded(session):
    ...
    session.add_all(
        [
            _show(1, "Mid", first_air_date=_TODAY + timedelta(days=30), popularity=50.0),
            _show(2, "Most Awaited", first_air_date=_TODAY + timedelta(days=10), popularity=99.0),
            _show(3, "Unscored", first_air_date=_TODAY + timedelta(days=20), popularity=None),
            _show(4, "Premiering Today", first_air_date=_TODAY, popularity=70.0),
            _show(5, "Already Out", first_air_date=_TODAY - timedelta(days=1), popularity=500.0),
            _show(
            ...
            _show(
                7,
                "Far Future",
                first_air_date=_TODAY + timedelta(days=ANTICIPATED_WINDOW_DAYS),
                popularity=300.0,
            ),
            _show(
                8,
                "Gone Upstream",
                first_air_date=_TODAY + timedelta(days=5),
                popularity=200.0,
...
async def test_status_is_not_in_the_predicate(session):
    """*Lanterns*: `Returning Series` with a future first air date belongs here."""
    session.add(
        _show(
            20,
            "Lanterns",
            first_air_date=_TODAY + timedelta(days=40),
...
async def test_the_default_length_caps_the_list(session):
    for offset in range(ANTICIPATED_LIMIT + 5):
        session.add(
            _show(
                100 + offset,
                f"Upcoming {offset:02d}",
                first_air_date=_TODAY + timedelta(days=1 + offset),
...
async def test_ties_break_deterministically(session):
    """Every future-dated show scores 0 until TMDB has anything to score, so the
    tie is the ordinary case here rather than the exotic one."""
    for show_id in (33, 31, 32):
        session.add(
            _show(
                show_id,
                f"Tied {show_id}",
                first_air_date=_TODAY + timedelta(days=7),
                popularity=0.0,
            )
        )
    await session.flush()
