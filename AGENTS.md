...
- `app` — user / auth / tracking / social data: `user`, `session`, `auth_token`, `login_attempt`,
  ...
  `user_episode_rating`, `connection`, `connection_request_log`, `activity_event`,
...
src/tvbf/
  ...
  app/
    ...
    services/          # account_service, my_shows_service, episode_service, invite_service, connection_service, reconciliation_service, auth_throttle (the IP-keyed signup/login gate, NEU-1160), connection_throttle (the per-requester outreach budget: `current_ceiling` selects between two `Throttle`s, `enforce` counts creations, `enforce_decline_cooldown` closes the targeted case, NEU-1157), report_service (persist, commit, *then* notify — the commit boundary is the contract, NEU-1162), handle_service (the one claimability rule both write sites need, the change-and-release transaction, and the change budget, NEU-1163)
...
**`app.watch_archive` was dropped in NEU-1158.** The archive (NEU-1029) was the TMDB migration's backstop: it described what a user watched in human terms — show name, premiere year, season, episode number and title, airdate — so a mapping failure stayed recoverable by hand. Its pre-drop `pg_dump` is the recovery path now. The four blocks this section had about its append-only nature, its trigger, its writer and its verification have been removed with the table.
