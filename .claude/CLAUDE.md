...
**The suite runs under `pytest-xdist` (`addopts = "-n auto"`), one database per worker.** `tests/conftest.py` derives `<test db>_<worker>` (e.g. `tvbf_test_gw3`) from `TEST_DATABASE_URL` and `PYTEST_XDIST_WORKER` and creates it on first use, so nothing else needs a per-worker name and the databases persist between runs. Isolation is a database apart rather than a schema apart because `app` and `catalog` are names the code binds to. `task test -- -n 0` runs serially.
